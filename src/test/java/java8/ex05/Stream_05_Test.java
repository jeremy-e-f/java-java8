...
public class Stream_05_Test {
    ...
    private long iterateSum(long n) {
        return Stream.iterate(1, nb -> nb+1).limit(n-1).collect(Collectors.summingLong(nb -> (long)nb));
        //return Stream.iterate(1, nb -> nb+1).limit(n-1).reduce((nb1,nb2)->nb1+nb2).get();
    }
